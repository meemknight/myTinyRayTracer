		...
		if (GetAsyncKeyState(i) == -32767)
		{
			...
			else if (i == VK_RETURN)
			{
				if(console->writeBuffer[0])
				{
					...
					console->writeText("\n");
					tokenizeCommand(console->writeBuffer, console, serializedVars, reload);
...
void tokenizeCommand(std::string msg, Console* console, SerializedVariabels* serializedVars, int& reload)
{
	...
	processCommand(firstToken, params, console, serializedVars, reload);
}


void processCommand(std::string& firstToken, std::vector<std::string>& params,
	Console* console, SerializedVariabels* serializedVars, int &reload)
{
	reload = 0;
