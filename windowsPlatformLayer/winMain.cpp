...
#pragma region draw console
			...
			drawConsole(&gameWindowBuffer, &platformFunctions.console, &gameInput, &gameMemory->serializedVariables, reload);
...
#pragma region check if game code changed	
		...
		if(CompareFileTime(&lastFileTime, &fileTime2) != 0 || reload)
		{
			reload = 0;
			...
			win32UnloadDll();
			HANDLE file;
			while ((file = CreateFile(dllName, GENERIC_READ | GENERIC_WRITE, NULL, NULL,
				OPEN_EXISTING, 0, NULL)) == INVALID_HANDLE_VALUE)
			{
				//check if the dll can be opened
			}

			CloseHandle(file);
			while (!win32LoadDll(&gameLogic_ptr, &onCreate_ptr, &onReload_ptr, &onClose_ptr, dllName)) 
			{}
