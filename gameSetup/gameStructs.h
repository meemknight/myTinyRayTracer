...
struct GameWindowBuffer
{
	...
	void drawAt(int x, int y, unsigned char r, unsigned char g, unsigned char b)
	{
		if (x >= w || y >= h || x < 0 || y < 0) 
			{ return; }
	...
	void drawAt(int x, int y, int r, int g, int b)
	{
		drawAt(x, y, (unsigned char)r, (unsigned char)g, (unsigned char)b);
	}

	void drawAt(int x, int y, float r, float g, float b)
	{
		auto convert = [](float c) -> unsigned char { glm::clamp(c, 0.f, 1.f); return c * 255u; };

		drawAt(x, y, convert(r), convert(g), convert(b));
	}
	
	void drawAt(int x, int y, glm::vec3 color)
	{
		drawAt(x, y, color.r, color.g, color.b);
	}

	void drawAt(int x, int y, glm::ivec3 color)
	{
		drawAt(x, y, (unsigned char)color.r, (unsigned char)color.g, (unsigned char)color.b);
	}
